...
CSI `external-provisioner` v6.2.0+ adds support for the following keys in `StorageClass.parameters`:
...
The CSI `external-provisioner` (v6.2.0+) looks for the following keys in `StorageClass.parameters`:
